...
from fastdeploy.worker.xpu_model_runner import xpu_pre_process, xpu_post_process, step_paddle, xpu_process_output
...
class XPUVLModelRunner(GPUVLModelRunner):
    ...
    #     # TODU(lilujia): to be fixed for multi queries
    #     # self.attn_backend.attention_metadata.rotary_embs = self.attn_backend.attention_metadata.rotary_embs[0]
    ...
    def _prepare_inputs(self) -> None:
        ...
        print(f'self.XPUModelRunner_cnt: {self.XPUModelRunner_cnt}, ids_remove_padding 01: {self.share_inputs["ids_remove_padding"]}')
        self.forward_meta.pos_emb_type = "HALF_HEAD_DIM"
        self.share_inputs["decoder_batch_ids"] = paddle.full(
            [self.fd_config.parallel_config.max_num_seqs, 1], 0, dtype='int32')
        self.share_inputs["decoder_tile_ids_per_batch"] = paddle.full(
    ...
    def generate(self) -> None:
        ...
        print(f'self.XPUModelRunner_cnt: {self.XPUModelRunner_cnt}, hidden_states 0, max: {paddle.max(hidden_states)}, min: {paddle.min(hidden_states)}, mean: {paddle.mean(hidden_states)}')
        print(f"shape 1 {hidden_states.shape}")
        hidden_states = xpu_process_output(hidden_states,
                                            self.share_inputs["cum_offsets"],
                                            self.forward_meta)
        print(f"shape 2 {hidden_states.shape}")
